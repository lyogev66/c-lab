...
{
	grep_options->size_of_regex_array++;
	if (grep_options->size_of_regex_array == INITIAL_ARRAY_SIZE) {
		grep_options->regex_array = (regex_struct*)malloc(INITIAL_ARRAY_SIZE * sizeof(regex_struct));
	}else{
		grep_options->regex_array = \
		(regex_struct*)realloc(grep_options->regex_array,(grep_options->size_of_regex_array) * sizeof(regex_struct));
	}
}
...
void insert_sqr_to_regex_array\
	(grep_options_struct *grep_options,type_of_character type,char starting_char,char ending_char)
{
	allocate_cell_for_regex_array(grep_options);
...
void remove_backslash_from_search_string(grep_options_struct *grep_options)
{
	...
	for(current_substring=grep_options->searched_str; current_substring[CURRENT_CHAR] != '\0'; current_substring++)
	{
...
{
	char *current_substring = NULL ,*temp = NULL;
